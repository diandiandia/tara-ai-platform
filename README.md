...
- **内容哈希唯一 ID 机制**：安全声称 ID (`CLM_` + Hash)、安全目标 ID (`CSO_` + Hash)、安全控制 ID (`CSC_` + Hash) 和安全要求 ID (`CSR_` + Hash) 统一为与 `DS_` 相同的 Hash 风格（基于内容生成，8位大写十六进制）。相同内容自动映射为相同 ID，确保跨不同模块和重新跑 TARA 时 ID 的绝对稳定与唯一。
- **网页端 ID 列隐藏（极致简洁）**：在 TARA 结果页的评估表和安全控制矩阵看板中，默认隐藏这些繁琐的 ID 编号列，保持界面平铺整洁美观；而在导出的 Excel 与 CSV 报表中完整留存，确保完美的安全追溯与审计合规。
- **增量分析与人工修改继承**：采用哈希指纹匹配，当资产属性未变化时自动跳过 AI 调用；且能完美继承安全员已确认或调整过的人工结论。
- **Excel 对齐评估表与控制矩阵**：全新设计“TARA 评估详情表”和“项目级安全控制要求矩阵”两大审阅看板，支持与导出 Excel 模板完全一致 of 平铺列展示与中英文多语言一键切换。
...
### 5. 高性能合规报告导出
- **Excel (.xlsx) 导出**：支持多工作表（Sheet）导出，包含 TARA 评估表、CSR 安全控制要求矩阵以及资产列表 `Assets` 三个 Sheet 页，具有自适应列宽与高级灰配色。
- **CSV 导出**：为满足单表纯文本归档规范，CSV 导出时仅包含 `TARA` 威胁分析评估详情，并采用 `utf-8-sig` (带有 BOM) 编码，解决 Windows Excel 打开中文时乱码的问题。
...
3. **会话超时自动注销与修改阻断**：系统在前端配置了 5 秒周期的 Token 过期轮询检查，并建立了全局 Axios 请求/响应拦截器。一旦发现登录超时或收到服务器端的 401 响应，系统将立即执行注销逻辑，清空前端 Session 缓存、弹出超时警告并重定向回登录页，并彻底拦截所有后续网络请求和页面修改行为，以保证敏感的 TARA 评估数据不被非授权更改。
