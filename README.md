...
- **自动 ID 编号与自增机制**：对安全声称 ID (`CLM-XXXX`)、安全目标 ID (`CSO-XXXX`)、安全控制 ID (`CSC-XXXX`)、安全要求 ID (`CSR-XXXX`) 进行自动编号和字母序自增规则的生成，保证跨多次导出的 ID 稳定性。
...
- **Excel 对齐评估表与控制矩阵**：全新设计“TARA 评估详情表 (与Excel对齐)”和“项目级安全控制要求矩阵”两大审阅看板，支持与导出 Excel 模板完全一致 of 平铺列展示与中英文多语言一键切换。
