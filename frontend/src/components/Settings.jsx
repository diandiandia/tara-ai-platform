  ...
  return (
    <div className="dashboard-container" style={{ maxWidth: '720px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <div>
          <h1 className="section-title" style={{ margin: 0 }}>系统配置</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginTop: '4px' }}>
            全局大模型接口及参数配置，支持一键校验结构化连通性 (BR-59, BR-71)。
          </p>
        </div>
      </div>
      ...
      {!isEditable && (
        <div style={{
          ...
          <AlertCircle size={16} />
          <span>您当前的账户角色为 “分析员”，仅可查看配置。如需修改，请使用 “管理员” 账户登录。</span>
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="glass" style={{ padding: '32px' }}>
        <div className="input-group">
          <span className="input-label" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Globe size={14} /> 大模型 API 地址 (API Base URL)
          </span>
          <input
            type="url"
        ...
        <div className="input-group">
          <span className="input-label" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Key size={14} /> API 密钥 (API Key / Token)
          </span>
          <input
            type="password"
            className="input-field"
            placeholder="请输入大模型授权密钥 (sk-...)"
            value={apiKey}
          ...
          <span style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>
            注: 若在沙盒内测试或不需要真实大模型调用，可以设置为 <b>mock_test_key</b>
          </span>
        </div>

        <div className="input-group" style={{ marginBottom: '32px' }}>
          <span className="input-label" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Layers size={14} /> 大模型名称 (Model Name)
          </span>
          <input
            type="text"
            className="input-field"
            placeholder="例如: gpt-4o, gpt-3.5-turbo, custom-model"
            value={modelName}
        ...
        {testResult && (
          <div style={{
            ...
            <div style={{ fontWeight: '700', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span>{testResult.success ? '连通性测试成功！' : '连通性测试失败！'}</span>
            </div>
          ...
          >
            {testing ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <div className="spinner"></div> 正在连通测试...
              </div>
            ) : '测试结构化连通性'}
          </button>
