...
3. **Session Timeout & Request Blocking Safety Guard**: A 5-second periodic token expiration checker runs on the frontend, combined with global Axios request and response interceptors. If a session timeout occurs or the backend responds with a 401 unauthorized status, the system instantly logs out, purges local session caches, alerts the user, redirects to `/login`, and blocks any subsequent data modification attempts, completely eliminating unauthorized overrides.
