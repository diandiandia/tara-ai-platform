...
本文档根据 `docs/requirements-frozen.md`（项目仓库内相对路径）中定义的 **TARA AI 平台 (v3 冻结版)** 整理而成，集中展现系统的业务流程、实体模型以及所有核心业务规则。
...
*   **风险决策联动 (BR-69)**：风险处置策略采用 ISO 21434 标准四值 `Avoid` (规避) / `Reduce` (缓解) / `Share` (转移) / `Retain` (接受)。仅 `Reduce` 触发第⑤阶段 CSO/CSR 生成；当决策（AI 或人工修改）为非 `Reduce`（即 `Avoid`/`Share`/`Retain`）时，系统在第⑤阶段自动免除该威胁场景的 CSR 生成。
