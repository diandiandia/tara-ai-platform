...
*   **Excel 对齐与项目级控制矩阵**：全新设计与导出的平铺 Excel 列完全对齐的“TARA 评估详情表 (与Excel对齐)”与由 7 列构成的“项目级安全控制要求矩阵”看板，支持行内多语言一键切换。
...
*   **自动 ID 编号与自增规则 (BR-80)**：为安全声称 ID (`CLM-XXXX`)、安全目标 ID (`CSO-XXXX`)、安全控制 ID (`CSC-XXXX`) 和安全要求 ID (`CSR-XXXX`) 设计自动分配机制。为了确保跨多次分析和导出的 ID 稳定性，系统在生成映射时需对域控下所有分析文本按字母进行升序排序，并由 `1` 开始顺序递增分配编号。
...
### 3.4 报告导出与系统配置
*   **导出与脱敏 (BR-57, BR-77)**：报告直接存储在本地服务器上。导出 Excel 时必须支持“导出脱敏版报告”选项，隐藏攻击路径细节和漏洞备注，仅保留最终安全需求（CSR）和确认后的风险等级。
