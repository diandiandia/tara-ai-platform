...
*   **自增 ID 排序与生成机制**：
    在 `backend/app/api/report.py` 中引入 `build_id_maps(steps)` 辅助函数。每次生成报表导出时，它会首先收集域控下所有非空的安全声称、安全目标 (CSO)、安全控制 (CSC) 与安全要求 (CSR) 文本。随后利用 Python 的 `sorted` 进行去重和字母排序，并在排序后按照 `CLM-XXXX`、`CSO-XXXX`、`CSC-XXXX` 和 `CSR-XXXX` (其中 `XXXX` 为 4 位补零数字，如 `0001` 开始) 分配唯一的顺序自增 ID。这能确保在输入和内容不变的情况下，导出的 ID 具备绝对的一致性与稳定性。
*   **网页端隐藏与报表留存设计**：
    在前端 React 展现层中，`TaraResults.jsx` 专门屏蔽了 CSO ID、CLM ID、Control ID 和 CSR ID 的展示，通过将此类列剔除或通过样式隐藏，确保页面布局紧凑易读。然而在执行 Excel/CSV 报表导出时，后端 `report.py` 的导出器会利用上述生成的 ID Map 将所有对应的编号字段写入每一行，从而在保证网页简洁的同时，使得导出的文档拥有 100% 可追溯和满足外部合规的结构设计。
...
*   **TARA 细节表 (与 Excel 对齐)**：展示已分析的损害场景、威胁场景、攻击可行性及风险处理决策（支持中英文多语言切换、逐项行内修改、手动新增或快捷整行删除，并在保存时原子化同步至数据库）。
*   **项目级汇总视图 (项目级安全控制要求矩阵)**：全新平铺矩阵表设计。展示与 Excel 导出完全一致的 7 列（`CSR ID`、`Security Domain`、`Asset SN`、`Asset Name`、`Requirement Title`、`Requirement Subtitle`、`Cybersecurity Requirement`），梳理本项目下所有域控生成的网络安全要求 (CSR) 和网络安全目标 (CSO)，提供完整的系统安全控制矩阵。
*   **CAF Level 标定覆盖**：计算引擎基于 AF Level 初始化并联动计算风险值，界面中支持安全专家下拉标定和覆盖（Calibration Override）最终的 CAF Level，且对大小写输入具有智能容错。
