...
*   **自增 ID 排序与生成机制**：
    为了确保在输入和内容不变的情况下，导出的 ID 具备绝对的一致性与稳定性，系统不再采用不稳定的顺序自增数字（如 `CSO-0001`），而是采用统一的**内容哈希（Content-based Hash）唯一 ID 机制**。在后端 `tasks.py` 和 `report.py` 中，所有安全声称（`CLM_`）、安全目标（`CSO_`）、安全控制（`CSC_`）及安全要求（`CSR_`）都会基于文本内容进行 SHA-256 并截取前 8 位生成哈希 ID（如 `CSO_A8B9C1D2`）。这样能保证相同的内容生成 100% 稳定的哈希 ID，与损害场景 SN 的生成逻辑相统一。
*   **网页端隐藏与报表留存设计**：
