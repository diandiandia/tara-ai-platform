...
*   **自动提取资产 (DFD Parser)**：
    *   用户点击【收集资产】时，后端解析 `snapshot_json`。
    *   **节点 (Nodes)** 映射为硬件资产（如 ECU 节点）、软件资产（如进程/服务）或数据资产（如存储节点）。
    *   **连线 (Edges)** 映射为通信/数据流资产，解析其协议字段。
    *   **更新保留逻辑**：
...
*   **当前域控内的 AI 资产去重**：
    ...
    *   后端聚合当前域控下的所有资产，过滤出候选去重列表。
    *   调用大模型，传入当前的资产列表（名称、类型、协议、描述），要求其识别名称拼写不同但实质相同的资产，并输出标准的 JSON 去重建议：
        ```json
        [
          {
            "keep_asset_id": 12,
